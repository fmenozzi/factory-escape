...
func _ready() -> void:
    ...
    # If the config file doesn't already exist, create it and populate it with
    # default options. Do so before connecting signal to avoid seeing a save
    # indicator.
    if not Options.has_config():
