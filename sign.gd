...
func modulate_label_visibility(old: Color, new: Color) -> void:
    var prop := 'modulate'
    var duration := 0.25
...
func _on_player_entered(player: Player) -> void:
    ...
    modulate_label_visibility(LABEL_NOT_VISIBLE, LABEL_VISIBLE)
...
func _on_player_exited(player: Player) -> void:
    ...
    modulate_label_visibility(LABEL_VISIBLE, LABEL_NOT_VISIBLE)
